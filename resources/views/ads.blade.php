        ...
        <nav aria-label="breadcrumb" class="bg-light">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a
                        href="{{ route('city', [$state->slug, $city->slug]) }}">{{ $city->name }}</a></li>
        ...
        @if (count($posts) === 0)
            <div class="alert alert-info alert-dismissible fade show" role="alert">
                ...
                Sorry there is no ad founded in <a class="text-red-700"
                    href="{{ route('city', [$state->slug, $city->slug]) }}">{{ $city->name }}</a> {{ $category->name }}
                section.
            </div>
        @else
                    ...
                    <nav aria-label="Page navigation example">
                        <ul class="pagination">
                            <li class="{{ $page == 1 ? ' disabled' : '' }} page-item">
                                <a class="page-link"
                                    href="{{ route('cityPosts', ['state' => $state->slug, 'city' => $city->slug, 'category' => $category->slug, 'page' => 1]) }}">Previous</a>
                            </li>
                            @foreach ($pageNumbers as $item)
                                <li class="{{ $page == $item ? ' active' : '' }} page-item">
                                    <a class="page-link"
                                        href="{{ route('cityPosts', ['state' => $state->slug , 'city' => $city->slug , 'category' => $category->slug, 'page' => $item]) }}">{{ $item }}</a>
                                </li>
                            @endforeach
                            ...
                            <li class="{{ $page == count($pageNumbers) ? ' disabled' : '' }} page-item">
                                <a class="page-link"
                                    href="{{ route('cityPosts', ['state' => $state->slug , 'city' => $city->slug , 'category' => $category->slug, 'page' => $page + 1]) }}">Next</a>
                            </li>
                        </ul>
                    </nav>
                @endif
